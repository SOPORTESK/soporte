-- =====================================================================
-- sek_append_hist_v2: versión mejorada de sek_append_hist.
--
-- NO REEMPLAZA a sek_append_hist. Es una función nueva que se prueba
-- de forma aislada antes de cambiar cualquier caller.
--
...
--      no hay messageId.
--
-- La función original (sek_append_hist) queda intacta.
-- =====================================================================

create or replace function public.sek_append_hist_v2(
  p_case_id        text,
  p_entry          jsonb,
  p_col            text,
...
begin
  if p_col not in ('histcliente', 'histtecnico') then
    raise exception 'sek_append_hist_v2: columna invalida %', p_col;
  end if;
...
grant execute on function public.sek_append_hist_v2(text, jsonb, text, text, text) to service_role;
