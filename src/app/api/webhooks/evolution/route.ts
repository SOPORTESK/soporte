...
function parseFileLength(fl: any): number {
  if (!fl) return 0;
  if (typeof fl === "number") return fl;
  if (typeof fl === "string") {
    const n = parseInt(fl, 10);
    return isNaN(n) ? 0 : n;
  }
  if (typeof fl === "object") {
    const low = Number(fl.low || 0);
    const high = Number(fl.high || 0);
    return (low >>> 0) + (high * 4294967296);
  }
  return 0;
}

// Extrae { url, directPath, mediaKey, mimetype, fileName, fileLength } del objeto de mensaje
function extractMediaInfo(msgObj: any): {
  ...
  return {
    ...
    fileLength: parseFileLength(media.fileLength),
  };
}
...
async function triggerSidecarOptimizer(params: {
  ...
  const OPTIMIZER_URL = process.env.VIDEO_OPTIMIZER_URL || "http://129.146.7.74/video-optimizer";
...
export async function POST(req: NextRequest) {
  ...
  let needsOptimizer = false;
  let optimizerBase64: string | undefined = undefined;
  let optimizerEncUrl: string | null = null;
  let optimizerMediaKey: any = null;
  ...
  if (mediaType && EVO_URL && EVO_KEY && EVO_INSTANCE) {
    try {
      ...
      optimizerEncUrl = encUrl;
      optimizerMediaKey = mediaInfo?.mediaKey;
      ...
      const isHeavyMedia = (mediaInfo?.fileLength ? mediaInfo.fileLength >= 25 * 1024 * 1024 : false);

      if (inlineB64) {
        base64 = String(inlineB64);
        console.log("[evo-webhook] base64 inline detectado en payload, longitud:", base64.length);
      } else if (isHeavyMedia && encUrl && mediaInfo?.mediaKey) {
        console.log("[evo-webhook] Archivo pesado detectado antes de descarga, delegando a VPS Sidecar:", {
        ...
        if (originalFileName && originalFileName.includes(".")) {
          ext = originalFileName.split(".").pop() || ext;
      ...
      if (!needsOptimizer && (base64 || directDecryptedBuffer)) {
        let dataStr = "";
        ...
        if (uploadBuffer.length >= 25 * 1024 * 1024) {
          console.log("[evo-webhook] Buffer pesado detectado (" + uploadBuffer.length + " bytes), delegando a VPS Sidecar...");
          needsOptimizer = true;
  ...
  try {
    ...
    // ── AUTO-RELLENO: buscar casos previos del mismo teléfono para reutilizar ÚNICAMENTE datos de contacto ──
    ...
    if (!isOutgoing && contactPhone) {
      ...
      if (prevCases && prevCases.length > 0) {
        // Buscar el caso más reciente que tenga datos de perfil del cliente (nombre, correo o cuenta)
        for (const pc of prevCases) {
          const pcCliente = (pc.cliente && typeof pc.cliente === "object") ? pc.cliente as Record<string, unknown> : {};
          if (pcCliente.nombre || pcCliente.correo || pcCliente.cuenta || pcCliente.cedula) {
            knownClient = {
              nombre: pcCliente.nombre || null,
              correo: pcCliente.correo || null,
              cuenta: pcCliente.cuenta || null,
              cedula: pcCliente.cedula || null,
            };
            console.log(`[evo-webhook] Auto-relleno de perfil de contacto para ${cleanPhone} — nombre: ${pcCliente.nombre || "N/A"}, correo: ${pcCliente.correo || "N/A"}, cuenta: ${pcCliente.cuenta || "N/A"}`);
            break;
          }
        }
      } else {
    ...
    if (isOutgoing) {
      ...
      if (needsOptimizer && nuevoCaseId) {
        await triggerSidecarOptimizer({
          ...
          mediaType: mediaType || "video",
          fileName,
          caseId: String(nuevoCaseId),
          messageId: keyId,
        });
      }

      if (hasKnownData) {
        console.log(`[evo-webhook] Caso ${nuevoCaseId} creado con perfil de contacto: nombre=${knownClient.nombre || "N/A"}, correo=${knownClient.correo || "N/A"}, cuenta=${knownClient.cuenta || "N/A"}`);
      }
